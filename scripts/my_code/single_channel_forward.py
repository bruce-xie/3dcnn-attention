...
l_fc1 = lasagne.layers.DenseLayer(
    ...
    W=voxnet.init.loadw3(),
    b=voxnet.init.loadb3(),
    name =  'fc1'
    )
l_fc2 = lasagne.layers.DenseLayer(
    ...
    num_units = n_classes,
    # W = lasagne.init.Normal(std = 0.01),
    W=voxnet.init.loadw4(),
    b=voxnet.init.loadb4(),
    nonlinearity = None,
    name = 'fc2'
    )
...
for i,(x_shared, y_shared) in enumerate(loader1):
    ...
    if l_out.name == 'fc2':
        if y_shared[0] == 0:
            plt.plot(rr[0, 0], rr[0, 1], 'bv')
        else:
            plt.plot(rr[0, 0], rr[0, 1], 'r^')
